...
def create_db(input_file, output_dir,
    ...
    single_label -- only save first label to db 
    valid_ratio -- ration of valid data, if 0, only create train db
    ...
    image_count = _fill_load_queue(input_file, load_queue, shuffle, single_label)
...
def _create_lmdb(image_count, write_queue, batch_size, output_dir,
    ...
    train_img_db_dir = os.path.join(output_dir, "train_img")
    train_label_db_dir = os.path.join(output_dir, "train_label")
    val_img_db_dir = os.path.join(output_dir, "val_img")
    val_label_db_dir = os.path.join(output_dir, "val_label")

    train_img_db = makedb(train_img_db_dir, lmdb_map_size)
    train_label_db = makedb(train_label_db_dir, lmdb_map_size)
    if valid_ratio:
        val_img_db = makedb(val_img_db_dir, lmdb_map_size)
        val_label_db = makedb(val_label_db_dir, lmdb_map_size)

    while (threads_done < num_threads) or not write_queue.empty():

        # Send update every 2 seconds
        if time.time() - wait_time > 2:
            logger.debug('Processed %d/%d' % (data_written, image_count))
            wait_time = time.time()
        ...
        if not write_queue.empty():
            img_datum, label_datum = write_queue.get()
            img_batch.append(img_datum)
            label_batch.append(label_datum)

            if len(img_batch) == batch_size:
                if data_written <= (image_count / 100 * (100 - valid_ratio)):
                    _write_batch_lmdb(train_img_db, img_batch, data_written)
                    _write_batch_lmdb(train_label_db, label_batch, data_written)
                else:
                    _write_batch_lmdb(val_img_db, img_batch, data_written)
                    _write_batch_lmdb(val_label_db, label_batch, data_written)
                data_written += len(img_batch)
                img_batch = []
                label_batch = []
            processed_something = True
    ...
    if len(img_batch) > 0:
        if valid_ratio:
            ...
            print "valid ratio is 0, all data save to train db"
            _write_batch_lmdb(train_img_db, img_batch, data_written)
            _write_batch_lmdb(train_label_db, label_batch, data_written)
        data_written += len(img_batch)

    if data_loaded == 0:
        raise LoadError('no data loaded from input file')
    logger.debug('%s data loaded' % data_loaded)

    if data_written == 0:
        raise WriteError('no data written to database')
    logger.info('%s data written to database' % data_written)

    if compute_mean:
        _save_means(image_sum, data_written, mean_files)
...
def _fill_load_queue(filename, queue, shuffle, single_label):
    """
    ...
    with open(filename) as infile:
        if shuffle:
            ...
            for line in lines:
                total_lines += 1
                try:
                    result = _parse_line(line, distribution, single_label)
                    valid_lines += 1
                    queue.put(result)
                except ParseLineError:
                    pass
...
def _parse_line(line, distribution, single_label):
    """
    ...
    # Expect format - [/]path/to/file.jpg 123 123 or [/]path/to/file.jpg -123.22 +123.11224 or [/]path/to/file.jpg 123 -123.2312
    #match = re.match(r'(.+)\s+([-+]?\d+(?:\.\d+)?)\s+([-+]?\d+(?:\.\d+)?)\s*$', line)
    #if match is None:
    #    raise ParseLineError
    #not fixed to float type
    #path = match.group(1)
    #label1 = int(match.group(2))
    #label2 = int(match.group(3))
    if not single_label:
        path, label1, label2 = line.split(" ")
    else:
        path, label1 = line.split(" ")

    distribution[label1] += 1

    if not single_label:
        return path, (label1, label2)
    else:
        return path, label1
...
def _load_thread(load_queue, write_queue, summary_queue,
                 image_width, image_height, image_channels,
                 resize_mode, image_folder, compute_mean, 
                 single_label, backend=None, encoding=None):
    """
    ...
    while not load_queue.empty():
        ...
        if backend == 'lmdb':
            img_datum = _array_to_img_datum(image, encoding)
            label_datum = _array_to_label_datum(label, single_label)
            write_queue.put((img_datum, label_datum))
...
def _array_to_img_datum(image, encoding):
    """
...
if __name__ == '__main__':
    ...
    parser.add_argument('-v', '--valid_ratio',
                        type=int,
                        default=25,
                        help='ratio of valid data, if 0, only create train db')
    ...
    parser.add_argument('--single_label',
                        action='store_true',
                        help='only save first label to db')
